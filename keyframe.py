...
class VisualOdometryPipeline:
    ...
    def __init__(self, camera_matrix, dist_coeffs, feature_extractor, feature_matcher, keyframe_criteria):
        ...
        # Store keyframe selection criteria
        self.keyframe_criteria = keyframe_criteria
        
        # Data from the last accepted keyframe
        self.last_keyframe_data = None
        self.last_keyframe_features_count = 0

        # Global pose (world rotation and translation)
        self.world_R = np.eye(3)
        self.world_t = np.zeros((3, 1))

    def _is_keyframe(self, relative_t, relative_R, num_matches):
        """
        Decides if the current frame should be a keyframe based on selection criteria.
        """
        # Criterion 1: Minimum translation since last keyframe
        translation = np.linalg.norm(relative_t)
        if translation > self.keyframe_criteria['min_translation']:
            print(f"    -> Keyframe Trigger: Translation ({translation:.4f} > {self.keyframe_criteria['min_translation']})")
            return True

        # Criterion 2: Minimum rotation since last keyframe
        # Convert rotation matrix to axis-angle to get rotation magnitude
    ...
    def process_frame(self, frame):
        """
        Processes a single frame. It will track, estimate pose, and then decide
        if the frame is a keyframe to be fully integrated into the map.
        """
        ...
        if len(matches) < 20: # Need a minimum number of matches to even consider pose
            print("    -> Frame Discarded: Not enough matches to track.")
            return None

        # Tentatively estimate the relative pose
        relative_R, relative_t, pts1, pts2 = self._estimate_pose(matches, self.last_keyframe_data['keypoints'], keypoints)
        ...
        if self._is_keyframe(relative_t, relative_R, len(matches)):
            # This is a keyframe, so we proceed with triangulation and map update
            ...
            if points_3d_relative is not None:
                ...
                # Update the last keyframe data to this new, accepted keyframe
                self.last_keyframe_data = {'image': gray_frame, 'keypoints': keypoints, 'descriptors': descriptors}
                self.last_keyframe_features_count = len(keypoints)
                return points_3d_world
        else:
            # Not a keyframe, discard it and do nothing
...
def main():
    ...
    final_map_path = os.path.join(output_dir, "final_keyframe_map.pcd")

    # --- NEW: Keyframe Selection Criteria ---
    ...
    KEYFRAME_CRITERIA = {
        "min_translation": 0.2,   # Minimum translation (in meters, assuming scaled world) to be a keyframe
        "min_rotation": 0.2,      # Minimum rotation (in radians) to be a keyframe
        "min_feature_ratio": 0.7  # Add keyframe if tracked features fall below this ratio of last keyframe's features
    }
